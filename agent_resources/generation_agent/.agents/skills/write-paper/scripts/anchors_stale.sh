...
#
# The write-paper preflight (stage 1a) uses this to decide whether to run
# the STYLE_DISTILLER. The distiller ONLY proposes; the operator gates the accept.
#
#   bash anchors_stale.sh <paper_workspace>
#
...
if [[ $# -ne 1 ]]; then
  echo "usage: anchors_stale.sh <paper_workspace>" >&2
  exit 2
fi

paper_workspace="$1"
anchors_dir="$paper_workspace/style/anchors"
marker="$paper_workspace/style/.distilled_at"
