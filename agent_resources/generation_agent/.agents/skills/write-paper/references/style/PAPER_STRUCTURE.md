...
`STYLE_GUIDE.md` §0 and the PRIME DIRECTIVE in `roles/ROLE_CONTRACT.md`, which always
...
selected source content* — the number of headline results, the depth of the
dependency graph of the load-bearing source results, and how much standing machinery the proofs share —
...
5. **The body — one or two sections.** If the result was stated and labeled in the
   introduction, refer to it there and give its proof without defining the label a
   second time (proof architecture, §5.4). A small supporting source result becomes
   an inline `Claim` proved on the spot; a supporting result large enough to deserve
   its own statement becomes a `\begin{lem}`
...
5. **Body sections — in dependency order.** Order so each result is stated after
   everything it depends on, following the selected source's dependency graph: preparatory lemmas
   ...
   When a headline theorem was stated and labeled in the introduction, open its body
   proof with "We now prove Theorem~\ref{thm:main}." and do not define the label again.
...
5. **Body — staged in dependency order, possibly grouped into parts.** Order from
   ...
   section opens per §5.6 with a one-sentence statement of its role. A promoted
   lettered theorem is proved at its body home by reference to its introduction
   label; the label is never defined twice.
...
- **Render every result from its selected source**, fully quantified and
...
- **Placement.** A headline result may be *promoted* into the introduction as a
  numbered (note/mid) or lettered (long) theorem. Define its label there exactly
  once, then refer to that theorem when giving the proof at its body home. Ordinary
  supporting results are stated once, in the body, where their hypotheses are available.
- **Choose each result's environment by the role the source result plays** (every claim is
  ...
  hypotheses; `rem` for a non-load-bearing observation. Promote a source result from inline
...
each proof sits at its result's body home, in source-dependency order (§5.6).
...
- **Order.** Sections follow the dependency graph of the load-bearing source results: a result
...
selected source and paper actually cite. All bibliography and citation **form** — the manual
