...
- **§0 is the non-negotiable floor** — the integrity rules (preserve the
  ...
  metadata). These live in the role contract (`roles/ROLE_CONTRACT.md`, the PRIME
...
The operator may use a per-paper copy to encode their own preferences, and may put
gold-standard exemplar papers under the workspace's `style/anchors/` directory
(see `style/anchors/README.md`). Where an anchor or a `PROJECT_BRIEF.md` per-paper rule
...
DIRECTIVE in `roles/ROLE_CONTRACT.md` (items 1–6)**, binding on every role. It always
...
- Any internal pipeline identifier, codename, source-map identifier, or hash in visible output.
